...
export default function Signup() {
  ...
  const submit = async (e) => {
    ...
    setErr("");
    try {
      const { data } = await authApi.signup({ display_name, phone, password });
  ...
  return (
    <div style={{maxWidth:420, margin:"24px auto", background:"#1e1e1e", padding:16, borderRadius:16}}>
      <h1 style={{fontSize:20, fontWeight:"700"}}>Регистрация</h1>
      <form onSubmit={submit} style={{display:"flex", flexDirection:"column", gap:12, marginTop:12}}>
        <input placeholder="Имя" value={display_name} onChange={e=>setName(e.target.value)} style={{padding:12, borderRadius:8, background:"#2a2a2a", color:"#fff", border:"1px solid #333"}} />
        ...
        <button style={{padding:12, borderRadius:8, background:"#e91e63", border:"none", color:"#fff"}}>Создать аккаунт</button>
      </form>
    </div>
  );
}
